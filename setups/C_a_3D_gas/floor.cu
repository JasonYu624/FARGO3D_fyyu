...
__global__ void Apply_Floor_Kernel(real *dens,
  ...
  if (fluidtype != GAS) {
    ...
    // Gas-preconditioning stage: EPSILON is effectively zero (e.g. 1e-20).
    // Keep passive dust exactly co-moving with gas to prevent pressureless
    // polar/theta CFL blow-ups from dynamically irrelevant tracer dust.
    if (epsilon <= (real)1.0e-15) {
      vx[mem] = vg_x;
      vy[mem] = vg_y;
#ifdef Z
      if (vz != NULL) vz[mem] = vg_z;
#endif
#ifdef ADIABATIC
      if (energy != NULL) energy[mem] = (real)0.0;
#endif
      return;
    }
  ...
  // ------------------------------------------------------------------
  if (relmedian_cap_flag == 1 && fluidtype != GAS) {
    // Re-declare gas velocities for this block
    real vg_x = gas_vx[mem];
    real vg_y = gas_vy[mem];
#ifdef Z
...
extern "C" void Floor_gpu() {
  ...
  Apply_Floor_Kernel<<<gridSize, blockSize>>>(
      ...
      (int)ANOMALY_CHECK_FLAG, (real)ANOMALY_CHECK_R_RATIO, (real)ANOMALY_CHECK_HILL_RATIO,
      // Relative median cap parameters
      (int)RELATIVE_MEDIAN_CAP_FLAG, (real)RELATIVE_MEDIAN_CAP_FACTOR
  );
