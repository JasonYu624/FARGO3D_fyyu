...
__global__ void Apply_Floor_Kernel(real *dens,
  ...
  if (fluidtype != GAS) {
    ...
    if (tv_cap_flag == 1) {
      ...
      bool d2g_gated = (rhod <= (real)2.0e4 * dust_abs_floor && dust_to_gas < tv_cap_d2g_max);
      ...
      if (apply_cap) {
        ...
        // NSH terminal velocity: v_rel = 2*St/(1+St^2) * eta * vK
        ...
        // Cap relative velocity, not absolute dust velocity
        dv_x = clamp_real(dv_x, -vrel_cap, vrel_cap);
        dv_y = clamp_real(dv_y, -vrel_cap, vrel_cap);
#ifdef Z
        dv_z = clamp_real(dv_z, -vrel_cap, vrel_cap);
#endif
